...
class SubDecoder(nn.Module):
    ...
    def autoregressive_sampling(
        ...
        # Your code here
        current_step_codes = torch.full((B, 1, self.n_codebooks), fill_value=161, dtype=torch.long, device=device)

        for step in range(self.n_codebooks):
            logits = self.forward(embedding.unsqueeze(1), current_step_codes)[:, :, step, :]
            next_code = sampling_fn(logits)

            current_step_codes[:, :, step] = next_code

        codes_sequence = current_step_codes
        # ^^^^^^^^^^^^^^
...
class TTSTransformer(nn.Module):
    ...
    def autoregressive_sampling(
        ...
        current_codes = torch.full((batch_size, 1, self.subdecoder.n_codebooks), fill_value=start_token, dtype=torch.long, device=device)
        codes_mask = torch.ones(batch_size, 1, dtype=torch.bool, device=device)
        ...
        for _ in range(max_size):
            ...
            next_codes = self.subdecoder.autoregressive_sampling(embeddings, sampling_fn)
            pred_codes.append(next_codes)
            if (next_codes == end_token).any():
                break
            current_codes = torch.cat([current_codes, next_codes], dim=1)
            codes_mask = torch.ones(batch_size, current_codes.shape[1], dtype=torch.bool, device=device)

        codes = torch.cat(pred_codes, dim=1)
        # ^^^^^^^^^^^^^^
